...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < N_ITERS; ++i) {
        ...
        startTime = CycleTimer::currentSeconds();
        // transpose B
        double *B2_T = (double *)malloc( n*k*sizeof( double ));
        _mm_prefetch((const char*)(&B2[0] + sizeof(double)*k*n), _MM_HINT_T2);
        for(int ib=0; ib<k; ib++){
            for(int jb=0; jb<n; jb++){
                B2_T[jb*k+ib] = B2[ib*n+jb];
            }
        }
        ispc::gemm_ispc(m, n, k, A2, B2_T, C2, alpha, beta);
        endTime = CycleTimer::currentSeconds();
        printf("%.2lfms\n", (endTime - startTime)*1000);
